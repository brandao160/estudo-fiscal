...
- **Lista de modelos com fallback** (`FREE_MODELS` no topo do arquivo): o proxy tenta os modelos da lista em ordem, passando pro próximo se um estiver indisponível/sem crédito/fora do ar (400/402/404/429/503) **ou lento demais** (`MODEL_TIMEOUT_MS`, 12s por tentativa — cada modelo tem um `AbortController` próprio), em vez de depender de um único modelo fixo ou deixar o usuário esperando indefinidamente. Ordem atual:
