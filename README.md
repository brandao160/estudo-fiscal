...
- **Lista de modelos com fallback** (`FREE_MODELS` no topo do arquivo): o proxy tenta os modelos da lista em ordem, passando pro próximo se um estiver indisponível/sem crédito/fora do ar (400/402/404/429/503) **ou lento demais** (`MODEL_TIMEOUT_MS`, 8s por tentativa — cada modelo tem um `AbortController` próprio), em vez de depender de um único modelo fixo ou deixar o usuário esperando indefinidamente. Ordem atual:
  1. `openrouter/free` (grátis, roteador automático da OpenRouter)
  2. `google/gemma-4-31b-it:free` (grátis, uso geral)
  3. `upstage/solar-pro4` — **pago, mas ínfimo** (~$0,03 de entrada + $0,12 de saída por *milhão* de tokens, sem "reasoning") — só entra em ação se os dois grátis acima estiverem indisponíveis/lentos, como rede de segurança pra sempre ter uma resposta rápida sem custo relevante.
  4. `nvidia/nemotron-3-super-120b-a12b:free`, `thinkingmachines/inkling-small:free`, `google/gemma-4-26b-a4b-it:free` (grátis, último recurso caso até o pago falhe).
