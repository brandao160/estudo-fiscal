...
- **Editais com mais de um cargo**: no modo "Com IA", antes de extrair o conteúdo a IA primeiro verifica se o edital cobre mais de um cargo/emprego com matérias específicas diferentes (`buildCargoDetectionPrompt`). Se detectar mais de um, mostra uma lista pra escolher qual cargo você vai prestar (`#edital-cargo-select`) e só then extrai — filtrando pra trazer só os conhecimentos básicos comuns + o específico daquele cargo, ignorando o conteúdo dos outros cargos. Se detectar só um cargo (ou não conseguir decidir), extrai direto sem perguntar nada. Isso consome uma chamada extra à IA por importação; o modo "Sem IA" não faz essa distinção (não tem como um parser de regras saber diferenciar cargos de forma confiável).
...
- **Lista de modelos grátis com fallback** (`FREE_MODELS` no topo do arquivo): a OpenRouter muda os modelos disponíveis de graça toda semana — um modelo específico pode simplesmente sair do ar e passar a responder "não está disponível gratuitamente". Por isso o proxy tenta uma lista em ordem (começando pelo roteador automático `openrouter/free`) e passa pro próximo se um estiver indisponível (400/404/429/503), em vez de depender de um único modelo fixo. Se todos da lista pararem de funcionar, atualize `FREE_MODELS` conferindo openrouter.ai/models (filtro "Price: Free").
