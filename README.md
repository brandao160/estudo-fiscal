...
- **Lista de modelos grátis com fallback** (`FREE_MODELS` no topo do arquivo): o proxy tenta os modelos da lista em ordem e passa pro próximo se um estiver indisponível/fora do ar (400/402/404/429/503), em vez de depender de um único modelo fixo. A OpenRouter muda os modelos disponíveis de graça toda semana — um modelo específico pode simplesmente sair do ar; se todos da lista pararem de funcionar, atualize conferindo openrouter.ai/models (filtro "Price: Free").
  - **Já testamos usar modelos pagos** (GPT-6 Astra, Qwen3.8 Max, Claude Fable 5.1) na frente da lista pra comparar qualidade — e voltamos atrás: o GPT-6 Astra sozinho consumiu ~$0,83 numa única chamada (é um modelo com "reasoning", que cobra um monte de tokens invisíveis de "raciocínio" como se fossem saída). Ruim demais pra uma cota compartilhada por qualquer visitante do site, então a lista está 100% grátis de novo. Se um dia quiser testar modelo pago de novo, cuidado especial com modelos de "reasoning" — o custo real pode ser bem maior do que o preço por token sozinho sugere.
