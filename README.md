...
- **Lista de modelos com fallback** (`FREE_MODELS` no topo do arquivo): o proxy tenta os modelos da lista em ordem e passa pro próximo se um estiver indisponível/sem crédito/fora do ar (400/402/404/429/503), em vez de depender de um único modelo fixo. A OpenRouter muda os modelos disponíveis de graça toda semana — um modelo específico pode simplesmente sair do ar; se todos da lista pararem de funcionar, atualize conferindo openrouter.ai/models.
  - **Atualmente tem 3 modelos PAGOS na frente da lista** (`openai/gpt-6-astra`, `qwen/qwen3.8-max-0902`, `anthropic/claude-fable-5.1`) — colocados de propósito pra comparar qualidade contra os modelos grátis. Um lote de 20 questões custa entre ~$0,03 (Qwen) e ~$0,30 (Astra/Fable) **na chave compartilhada usada por todo mundo que visita o site**, não só de quem está testando — com um "Key Limit" de $1 configurado na chave, isso esgota em poucos lotes. Os modelos grátis (`openrouter/free`, `google/gemma-4-31b-it:free`, `nvidia/nemotron-3-super-120b-a12b:free`) continuam depois na lista como fallback. Pra voltar a ser 100% grátis, é só remover as 3 linhas dos modelos pagos do topo de `FREE_MODELS`.
